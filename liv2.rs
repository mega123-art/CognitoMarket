...
pub mod prediction_market {
    ...
    pub fn claim_winnings(ctx: Context<ClaimWinnings>) -> Result<()> {
        ...
        // In AMM, when you buy YES shares, they come from the NO pool (NO liquidity decreases)
        // When you buy NO shares, they come from the YES pool (YES liquidity decreases)
        
        // Total shares issued = how much the opposite pool decreased
        let total_winning_shares = if outcome_yes {
            // YES shares were issued by decreasing NO pool
            // Total YES shares = Initial NO - Current NO
            market.initial_liquidity
                .checked_sub(market.no_liquidity)
                .ok_or(ErrorCode::MathOverflow)?
        } else {
            // NO shares were issued by decreasing YES pool
            // Total NO shares = Initial YES - Current YES
            market.initial_liquidity
        ...
        // Total payout pool = All the funds in vault from both initial liquidity AND user purchases
        // This includes: (1) Initial liquidity of losing side, (2) All user deposits that went into losing side
        ...
        let payout = (winning_shares as u128)
            .checked_mul(total_winning_shares as u128)
            .ok_or(ErrorCode::MathOverflow)?
            .checked_div(total_winning_shares as u128)
            .ok_or(ErrorCode::MathOverflow)?;
