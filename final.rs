...
pub mod prediction_market {
    ...
    pub fn claim_winnings(ctx: Context<ClaimWinnings>) -> Result<()> {
        ...
        // Total winning shares issued is the *change* in the opposite pool's liquidity 
        // from the initial state. We must check the subtraction order to prevent underflow.
        let total_winning_shares = if outcome_yes {
            // FIX: Flip subtraction order for YES outcome since market.no_liquidity
            // is often greater than initial_liquidity, causing underflow.
            market.no_liquidity
                .checked_sub(market.initial_liquidity)
                .ok_or(ErrorCode::MathOverflow)?
        } else {
            // NO outcome (market.initial_liquidity > market.yes_liquidity in test)
        ...
        // REMOVED: Redundant/incorrect proportional calculation that was here
